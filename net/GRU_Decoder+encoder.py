...
class BST(paddle.nn.Layer):
    def __init__(self, user_count, item_emb_size, cat_emb_size,
        ...
        # Transformer encoder
        self.encoder_layer = paddle.nn.TransformerEncoderLayer(self.d_model, self.n_head, self.d_inner_hid)

        self.transformer_block = paddle.nn.TransformerEncoder(self.encoder_layer, 1)
    ...
    def forward(self, userid, hist_item_seq, hist_cat_seq, position_seq,
        ...
        '''
        enc_output = self.transformer_block(enc_output)

        # 256,429,288
        # Gru layer, only historical information needed
        Gru_output = item_sequence
        # 2,256,288
        Gru_output = self.GRU_layer(Gru_output)[1]
        # Gru_output = paddle.add(Gru_output[0], Gru_output[1])
        Gru_output = paddle.sum(Gru_output[1], axis=0)
        Gru_output = paddle.reshape(Gru_output, [len(enc_output), -1, self.d_model])
